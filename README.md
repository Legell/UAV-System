...
Flask Web UI — это простой сервер, который принимает телеметрию по MAVLink и отображает данные через веб-интерфейс.

---

### 1️⃣ Установка и распаковка файлов проекта
Скачайте файлы из репозитория и распакуйте проект в любую удобную директорию  
(например, `~/uav-web-ui` или `C:\uav-web-ui`).

### 2️⃣ Установка зависимостей
В командной строке (Linux/Windows) выполните:
...
Откройте `app.py` в любом удобном текстовом редакторе и измените значение переменной на 17-й строке  
`vpn_ip_repka = '127.0.0.1'`  
на IP-адрес Repka Pi внутри VPN-сети Tailscale.

### 4️⃣ Запуск Flask Web UI
Запустите `app.py` через интерпретатор Python в терминале (CMD/Terminal), а затем откройте в браузере страницу по адресу **`http://localhost:5555`**.
